...
const [apiVersion, source, sourceArg, regSource, regSourceArg] = process.argv.slice(2, 7);
let describeResult;
...
let regSourceResult;
if (regSource === '-r') {
  const regSourceFilePath = !path.isAbsolute(regSourceArg) ? path.resolve(process.cwd(), regSourceArg) : regSourceArg;
  regSourceResult = JSON.parse(fs.readFileSync(regSourceFilePath));
} else {
  printHelp();
  process.exit();
}
...
run('Applying registry updates', () => {
  const registry = regSourceResult;
