...
async function updateProfiles(profiles, retrievePackageDir, forceSourcePull) {
    if (forceSourcePull) {
        ...
        pjson.Package.types[pjson.Package.types.findIndex((x) => x.name.toString() === 'CustomObject')].members =
            pjson.Package.types[pjson.Package.types.findIndex((x) => x.name.toString() === 'CustomObject')].members.concat(config_1.default(await getProjectPath()).ensureObjectPermissions);
