...
async function sourcemove(movesources, root, filter) {
    ...
    for (const filepath of movesources) {
        exports.debug(`move file(s): ${filepath[0]} to ${filepath[1]}`);
                    ...
                    if (fix[workarounds][workaround].isactive === true) {
                        if (fix[workarounds][workaround].files) {
                            ...
                            if (fix[workarounds][workaround].files.move) {
                                ...
                                updatedfiles[workarounds + '/' + workaround] = updatedfiles[workarounds + '/' + workaround].concat(await sourcemove(fix[workarounds][workaround].files.move, root, filter));
                            }
