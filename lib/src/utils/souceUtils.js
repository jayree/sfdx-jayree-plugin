...
exports.updateProfiles = exports.applySourceFixes = exports.applyFixes = exports.getConnectionFromArgv = exports.logMoves = exports.logFixes = exports.moveSourceFilesByFolder = exports.profileElementInjection = exports.shrinkPermissionSets = exports.getProjectPath = exports.debug = void 0;
...
async function getProjectPath() {
    ...
    try {
        projectPath = slash_1.default(await core.SfdxProject.resolveProjectPath());
        return projectPath;
    }
    catch (error) {
        return undefined;
        ...
        if (await fs.pathExists(file)) {
            ...
            if (data[mutingOrPermissionSet].fieldPermissions) {
                exports.debug({
                    ...
                    removedFieldPermsissions: JSON.stringify(data[mutingOrPermissionSet].fieldPermissions.filter((el) => el.editable.toString() === 'false' && el.readable.toString() === 'false')),
                });
                data[mutingOrPermissionSet].fieldPermissions = data[mutingOrPermissionSet].fieldPermissions.filter((el) => el.editable.toString() === 'true' || el.readable.toString() === 'true');
    ...
    for (const filepath of movesources) {
        ...
        for (const file of files) {
            if (await fs.pathExists(file)) {
                ...
                exports.debug(`move file(s): ${from} to ${destinationFile}`);
                await fs.ensureDir(path.dirname(destinationFile));
...
async function getConnectionFromArgv() {
    ...
    const org = await core_1.Org.create({ aliasOrUsername });
            ...
            if (await fs.pathExists(file)) {
                ...
                if (fixsources[filename].insert) {
                    for (const inserttask of fixsources[filename].insert) {
                        ...
                        if (inserttask.value && !compareobj(object_path_1.objectPath.get(data, inserttask.path), inserttask.value)) {
                            object_path_1.objectPath.set(data, inserttask.path, `${inserttask.value}`);
                            fs.writeFileSync(file, builder.buildObject(data) + '\n' // .replace(/ {2}/g, '    ')
                            );
                            array.push({
                                filePath: file,
                            ...
                            exports.debug(`insert: Object ${JSON.stringify(inserttask.value)} found at ${inserttask.path}`);
                        }
                    }
                }
...
async function updateProfiles(profiles, retrievePackageDir, forceSourcePull) {
    if (forceSourcePull) {
        ...
        pjson.Package.types[pjson.Package.types.findIndex((x) => x.name.toString() === 'CustomObject')].members = pjson.Package.types[pjson.Package.types.findIndex((x) => x.name.toString() === 'CustomObject')].members.concat(config_1.default(await getProjectPath()).ensureObjectPermissions);
        ...
        let stdout;
        try {
            stdout = (await execa_1.default('sfdx', [
    ...
    if (await fs.pathExists(adminprofile)) {
        ...
        if (profileContent.Profile.objectPermissions) {
            profileElementInjectionFromAdmin.ensureObjectPermissions = profileContent.Profile.objectPermissions.map((el) => el.object.toString());
        }
    }
    else {
