...
async function updateProfiles(profiles, customObjects, forceSourcePull) {
    if (forceSourcePull) {
        ...
        exports.debug({ packageFilePath, destRoot });
        try {
            const pjson = await xml2js.parseStringPromise(await fs.readFile(path.join(__dirname, '..', '..', '..', 'manifest', 'package-profiles-only.xml'), 'utf8'));
            const index = pjson.Package.types.findIndex((x) => x.name.toString() === 'CustomObject');
            pjson.Package.types[index].members = pjson.Package.types[index].members.concat(config_1.default(await getProjectPath()).ensureObjectPermissions);
            await fs.emptyDir(destRoot);
            await fs.writeFile(packageFilePath, builder.buildObject(pjson));
            const componentSet = await source_deploy_retrieve_1.ComponentSet.fromManifest(packageFilePath);
    ...
    await profileElementInjection(profiles.map((profile) => profile.filePath).filter(Boolean), profileElementInjectionFromAdmin, customObjectsFilter);
}
