...
const colorette = (0, colorette_1.createColors)({ useColor: process.env?.LISTR_DISABLE_COLOR !== '1' });
...
class MyDefaultRenderer {
        ...
        switch (this.options.formatOutput) {
            case 'truncate':
                parsedStr = str.split(os_1.EOL).map((s, i) => {
                    return (0, cli_truncate_1.default)(MyDefaultRenderer.indentMultilineOutput(s, i), columns);
                });
                break;
            case 'wrap':
    ...
    getSymbol(task, data = false) {
        if (task.isPending() && !data) {
            ...
            return this.options?.lazy ? colorette.red(listr2_1.figures.warning) : colorette.red(this.spinner[this.spinnerPosition]);
        }
