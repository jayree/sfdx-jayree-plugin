...
class MyDefaultRenderer {
    ...
    constructor(tasks, options, renderHook$) {
        ...
        this.currentTasks = {};
        this.hiddenTasks = {};
        this.options = Object.assign(Object.assign({}, MyDefaultRenderer.rendererOptions), this.options);
    }
    getTaskOptions(task) {
        return Object.assign(Object.assign({}, MyDefaultRenderer.rendererTaskOptions), task.rendererTaskOptions);
    }
    isBottomBar(task) {
    ...
    multiLineRenderer(tasks, id = 'root', level = 0) {
        ...
        if (this.hiddenTasks[id].length > 0 && tasks.filter((t) => t.isPending()).length !== 0) {
            ...
            if (completed.length > 0) {
                output = [
                    ...output,
                    this.formatString(`... completed (${completed.length})`, chalk_1.default.green(figures_1.default.main.tick), level),
                ];
            }
            const failed = this.hiddenTasks[id].filter((t) => t.hasFailed());
            if (failed.length > 0) {
                output = [...output, this.formatString(`... failed (${failed.length})`, chalk_1.default.red(figures_1.default.main.cross), level)];
            }
            const skipped = this.hiddenTasks[id].filter((t) => t.isSkipped());
            if (skipped.length > 0) {
                output = [
                    ...output,
                    this.formatString(`... skipped (${skipped.length})`, chalk_1.default.yellow(figures_1.default.main.arrowDown), level),
                ];
            }
        }
                ...
                if (task.isEnabled()) {
                    ...
                    if (task.hasTitle()) {
                        ...
                        (task) => task.hasFailed()) &&
                            ...
                            if (task.hasFailed() && this.options.collapseErrors) {
                                ...
                                output = [
                                    ...output,
                                    this.formatString(task.message.error && this.options.showErrorMessage ? task.message.error : task.title, this.getSymbol(task), level),
                                ];
                            }
                                ...
                                output = [
                                    ...output,
                                    this.formatString((task.message.skip && this.options.showSkipMessage ? task.message.skip : task.title) +
                                        (this.options.suffixSkips ? chalk_1.default.dim(' [SKIPPED]') : ''), this.getSymbol(task), level),
                                ];
                            }
                                ...
                                output = [
                                    ...output,
                                    this.formatString(`${task === null || task === void 0 ? void 0 : task.title} ${this.getTaskTime(task)}`, this.getSymbol(task), level),
                                ];
                            }
                            else {
                                // normal state
                    ...
                    if (task.isCompleted() || task.hasFailed() || task.isSkipped()) {
                        ...
                        if (!this.taskTime[id][task.id]) {
                            this.taskTime[id][task.id] = time_span_1.default();
                        }
        ...
        if (level > 0 && this.currentTasks[id].length - this.options.maxSubTasks > 0) {
            output = [
                ...output,
                this.formatString(`... pending (${this.currentTasks[id].length - this.options.maxSubTasks})`, chalk_1.default.dim(figures_1.default.main.squareSmallFilled), level),
            ];
        }
        if (output.length > 0) {
    ...
    getSymbol(task, data = false) {
        ...
        if (task.isPending() && !data) {
            return ((_a = this.options) === null || _a === void 0 ? void 0 : _a.lazy) ||
                (this.options.showSubtasks !== false &&
                    task.hasSubtasks() &&
