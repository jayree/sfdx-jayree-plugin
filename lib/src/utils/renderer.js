...
class MyDefaultRenderer {
    ...
    render() {
        ...
        const updateRender = () => (0, log_update_1.default)(this.createRender());
        /* istanbul ignore if */
                    ...
                    if (task.hasTitle()) {
                        ...
                        (task) => task.hasFailed()) &&
                            ...
                            if (task.hasFailed() && this.getSelfOrParentOption(task, 'collapseErrors')) {
                                ...
                                output = [
                                    ...output,
                                    this.formatString(this.addSuffixToMessage(task.message.skip && this.getSelfOrParentOption(task, 'showSkipMessage')
                                        ? task.message.skip
                                ...
                                output = [
                                    ...output,
                                    this.formatString(this.addSuffixToMessage(task.title, `RETRYING-${task.message.retry.count}`), this.getSymbol(task), level),
                                ];
                            }
        ...
        switch (this.options.formatOutput) {
            case 'truncate':
                parsedStr = str.split(os_1.EOL).map((s, i) => {
                    return (0, cli_truncate_1.default)(this.indentMultilineOutput(s, i), columns);
                });
                break;
            case 'wrap':
