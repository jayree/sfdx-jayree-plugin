...
exports.analyzeFile = exports.getGitResults = exports.ensureDirsInTempProject = exports.appendToManifest = exports.convertTempProject = exports.addFilesToTempProject = exports.prepareTempProject = exports.debug = void 0;
...
const describe = require("../../metadata/describe.json");
...
async function convertTempProject(convertpath, options = { destruct: false }, task, ctx) {
    ...
    do {
        try {
            result = await execa('sfdx', ['force:source:convert', '--sourcepath', ctx.sfdxProjectFolders.toString(), '--json'], {
                cwd: convertpath,
            });
        }
        catch (e) {
...
async function getGitResults(task, ctx) {
    ...
    let gitLines = (await execa('git', ['--no-pager', 'diff', '--name-status', '--no-renames', ctx.git.ref1ref2])).stdout.split(/\r?\n/);
        ...
        if (line.status === 'D') {
            for (const sfdxFolder of ctx.sfdxProjectFolders) {
                ...
                if (line.path.startsWith(sfdxFolder)) {
                    extf = sfdxFolder;
                    if (line.path.startsWith(path_1.join(sfdxFolder, '/main/default/'))) {
                        extf = path_1.join(sfdxFolder, '/main/default/');
                    }
                    else {
                        extf = path_1.join(sfdxFolder, '/');
                    }
