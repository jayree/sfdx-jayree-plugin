...
function analyzeFile(path, ctx) {
    let source = '';
    let target = '';
    if (parseMetadataXml(path)) {
        try {
            const ref2resolver = new resolve_1.MetadataResolver(registryAccess, ctx.ref2VirtualTreeContainer);
        ...
        try {
            const ref1resolver = new resolve_1.MetadataResolver(registryAccess, ctx.ref1VirtualTreeContainer);
...
function getGitResults(task, ctx) {
    const results = {
        added: [],
