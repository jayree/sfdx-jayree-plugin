...
async function appendToManifest(file, insert, options = { destruct: false }) {
        ...
        childTypes.forEach((childType) => {
            if (types[childType]) {
                types[childType].forEach((ruleName) => {
                    ...
                    process.once('exit', () => {
                        cli_ux_1.cli.warn(`'${childType}:${ruleName}' replaced with '${masterType}:${masterLabel}' in ${path_1.basename(file)}`);
                    });
                });
    ...
    if (!options.destruct) {
        ...
        replaceChildwithParentType(['SharingOwnerRule', 'SharingCriteriaRule', 'SharingGuestRule', 'SharingTerritoryRule'], 'SharingRules');
