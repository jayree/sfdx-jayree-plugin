...
exports.PuppeteerStateTasks = void 0;
...
const CSconfig = tslib_1.__importStar(require("../../../config/countrystate.json"));
...
class PuppeteerStateTasks {
    ...
    async setCountryIntegrationValue() {
        ...
        const editIntValResult = await PuppeteerStateTasks.setHTMLInputElementValue(page, setCountrySelector.editIntVal, this.countrycode);
    ...
    async executeAdd() {
        ...
        const editIsoCodeResult = await PuppeteerStateTasks.setHTMLInputElementValue(page, selector.editIsoCode, stateIsoCode);
        const editIntValResult = await PuppeteerStateTasks.setHTMLInputElementValue(page, selector.editIntVal, stateintVal);
        const editActiveResult = await PuppeteerStateTasks.setHTMLInputElementChecked(page, selector.editActive, true, false);
        const editVisibleResult = await PuppeteerStateTasks.setHTMLInputElementChecked(page, selector.editVisible, true, true);
    ...
    async executeDeactivate() {
        ...
        const editVisibleResult = await PuppeteerStateTasks.setHTMLInputElementChecked(page, selector.editVisible, false, false);
        const editActiveResult = await PuppeteerStateTasks.setHTMLInputElementChecked(page, selector.editActive, false, false);
    ...
    async open() {
        if (!this.browser) {
            this.browser = await puppeteer_1.default.launch(config_1.default().puppeteer);
...
exports.PuppeteerStateTasks = PuppeteerStateTasks;
