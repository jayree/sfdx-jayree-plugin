...
const changelog = async function () {
    process.once('exit', () => {
        ...
        try {
            ...
            const changelogFile = fs.readFileSync(path_1.join(moduleRootPath, 'CHANGELOG.md'), 'utf8');
            const packageJson = fs.readJSONSync(path_1.join(moduleRootPath, 'package.json'));
            let changelogText;
            try {
                ...
                if (changelogText.length > 0) {
                    ...
                    console.log(marked_1.default('# CHANGELOG (sfdx-jayree)'));
                    // eslint-disable-next-line no-console
                    console.log(marked_1.default(changelogText));
                }
