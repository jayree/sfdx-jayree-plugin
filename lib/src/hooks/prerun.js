...
const stateFolderHandler_1 = require("../utils/stateFolderHandler");
...
const prerun = async function (options) {
    ...
    if (options.Command.id === 'force:source:pull') {
        if (kit_1.env.getBoolean('SFDX_ENABLE_JAYREE_HOOKS_RESET_BEFORE_PULL', false)) {
            ...
            const project = core_2.SfProject.getInstance(projectPath);
            const userName = (await (0, souceUtils_1.getConnectionFromArgv)()).username;
            const org = await core_2.Org.create({ aliasOrUsername: userName });
            try {
                storedServerMaxRevisionCounterPath = await (0, stateFolderHandler_1.getCurrentStateFolderFilePath)(projectPath, path.join('orgs', org.getOrgId(), 'jayreeStoredMaxRevision.json'), true);
            ...
            if (storedServerMaxRevisionCounter >= 0) {
                try {
                    ...
                    core_1.CliUx.ux.log(`Reset local tracking files to revision ${storedServerMaxRevisionCounter}.`);
                    // eslint-disable-next-line no-empty
                }
                catch (error) { }
            }
                ...
                try {
                    const { serverMaxRevisionCounter } = await fs.readJSON(await (0, stateFolderHandler_1.getCurrentStateFolderFilePath)(projectPath, path.join('orgs', (await core_2.Org.create({ aliasOrUsername: userName })).getOrgId(), 'maxRevision.json'), false), { throws: false });
                    localServerMaxRevisionCounter = serverMaxRevisionCounter;
                    // eslint-disable-next-line no-empty
                }
                catch (error) { }
                const answer = await core_1.CliUx.ux.confirm(chalk_1.default.dim(`WARNING: No stored revision found for scratch org with name: ${userName}.
