    ...
    async run() {
        ...
        const tasks = new listr2_1.Listr([
            ...
            },
            {
                ...
                skip: (ctx) => ctx.gitLines.length === 0,
                task: (ctx, task) => {
                    ctx.gitResults = (0, gitdiff_1.getGitResults)(task, ctx.gitLines, ctx.ref1VirtualTreeContainer, ctx.ref2VirtualTreeContainer);
                },
                options: { persistentOutput: true },
            },
            {
                ...
                task: (ctx, task) => task.newListr([
                    {
                        title: !ctx.gitResults.deleted.length && !Object.keys(ctx.gitResults.modified.toDestructiveChanges).length
                            ? undefined
                            : (0, path_1.join)('destructiveChanges', 'destructiveChanges.xml'),
                        // eslint-disable-next-line @typescript-eslint/no-shadow
                        ...
                        task: (ctx, task) => task.newListr([
                            {
                                ...
                                // eslint-disable-next-line @typescript-eslint/no-shadow
                                task: (ctx, task) => {
                                    ctx.destructiveChangesComponentSet = (0, gitdiff_1.createManifest)(ctx.ref1VirtualTreeContainer, { destruct: true }, ctx.gitResults, task);
                                },
                                options: {
                                    bottomBar: 5,
  ...
  <version>${ctx.sourceApiVersion}</version>
                    ...
                    },
                    {
                        title: !ctx.gitResults.added.length && !Object.keys(ctx.gitResults.modified.toManifest).length
                            ? undefined
                            : (0, path_1.join)('package', 'package.xml'),
                        // eslint-disable-next-line @typescript-eslint/no-shadow
                        ...
                        task: (ctx, task) => task.newListr([
                            {
                                ...
                                // eslint-disable-next-line @typescript-eslint/no-shadow
                                task: (ctx, task) => {
                                    ctx.manifestComponentSet = (0, gitdiff_1.createManifest)(ctx.ref2VirtualTreeContainer, { destruct: false }, ctx.gitResults, task);
                                },
                                options: {
                                    bottomBar: 5,
                                    persistentOutput: true,
