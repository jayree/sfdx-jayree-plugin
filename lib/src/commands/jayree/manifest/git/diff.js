        ...
        const tasks = new listr2_1.Listr([
            ...
            },
            {
                ...
                task: async (ctx, task) => {
                    ...
                    ctx.gitLines = await gitdiff_1.getGitDiff(ctx);
                },
                options: { persistentOutput: true },
            },
            {
            ...
            },
            {
                ...
                task: (ctx, task) => {
                    ctx.gitResults = gitdiff_1.getGitResults(task, ctx);
                },
                options: { persistentOutput: true },
            },
            {
                ...
                task: (ctx, task) => task.newListr([
                    {
                        title: !ctx.gitResults.deleted.length && !Object.keys(ctx.gitResults.modified.toDestructiveChanges).length
                            ? undefined
                            : `Generate ${path_1.join('destructiveChanges', 'destructiveChanges.xml')}`,
                        // eslint-disable-next-line @typescript-eslint/no-shadow
                        ...
                        task: (ctx, task) => task.newListr([
                            ...
                            {
                                // title: 'Save',
                                skip: () => !ctx.destructiveChangesComponentSet.size,
                                // eslint-disable-next-line @typescript-eslint/no-shadow
                                task: async (ctx) => {
  ...
  <version>${ctx.sfdxProject.sourceApiVersion}</version>
                    ...
                    },
                    {
                        title: !ctx.gitResults.added.length && !Object.keys(ctx.gitResults.modified.toManifest).length
                            ? undefined
                            : `Generate ${path_1.join('package', 'package.xml')}`,
                        // eslint-disable-next-line @typescript-eslint/no-shadow
                        ...
                        task: (ctx, task) => task.newListr([
                            ...
                            {
                                // title: 'Save',
                                skip: () => !ctx.manifestComponentSet.size,
                                // eslint-disable-next-line @typescript-eslint/no-shadow
                                task: async (ctx) => {
