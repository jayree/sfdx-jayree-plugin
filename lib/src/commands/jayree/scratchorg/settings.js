...
class ScratchOrgSettings extends command_1.SfdxCommand {
    ...
    async run() {
        ...
        try {
            ...
            out = json(shell.exec(`sfdx force:source:retrieve --manifest=${path.join(__dirname, '..', '..', '..', '..', '..', 'manifest', 'package-settings.xml')} --targetusername=${this.org.getUsername()} --apiversion=${apiVersion} --json`, {
                cwd: orgretrievepath,
                fatal: false,
                silent: true,
                env: Object.assign(Object.assign({}, process.env), { FORCE_COLOR: 0, SFDX_DISABLE_JAYREE_HOOKS: true }),
