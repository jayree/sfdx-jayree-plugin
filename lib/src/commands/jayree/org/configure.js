...
class ConfigureOrg extends command_1.SfdxCommand {
    async run() {
        ...
        const configPath = await command_1.core.fs.traverseForFile(process.cwd(), '.sfdx-jayree.json');
        let allTasks = [];
        let selectedSetupTasks = [];
        if (this.flags.tasks) {
            this.flags.tasks.forEach((task) => {
                selectedSetupTasks = selectedSetupTasks.concat(config_1.default(configPath).setupTasks.filter((t) => task === t.title));
            });
            allTasks = selectedSetupTasks;
        }
        else {
            selectedSetupTasks = config_1.default(configPath).setupTasks.filter((t) => t.isactive === true);
            allTasks = config_1.default(configPath).setupTasks;
        }
