...
const messages = command_1.core.Messages.loadMessages('sfdx-jayree', 'scratchorgtrackinglist');
class SourceTrackingList extends command_1.SfdxCommand {
    async run() {
        ...
        const { records } = await conn.tooling.query(`SELECT RevisionCounter,RevisionNum,Id,MemberType,MemberName,IsNameObsolete from SourceMember where RevisionCounter >= ${this.flags.revision}`);
        let sourceMemberResults = records.map((SourceMember) => {
            ...
            return Object.assign(Object.assign({}, SourceMember), { RevisionCounter, RevisionCounterString: `${RevisionCounter}${RevisionCounter === maxRev ? ' [remote]' : ''}${RevisionCounter === maxrevfile ? ' [local]' : ''}${RevisionCounter === storedServerMaxRevisionCounter ? ' [stored]' : ''}` });
        });
        ...
        this.ux.styledHeader(chalk_1.default.blue(`SourceMember revision counter numbers list for: ${this.org.getUsername()}`));
