...
class SourceTrackingList extends command_1.SfdxCommand {
    async run() {
        ...
        const maxrevpath = await (0, stateFolderHandler_1.getCurrentStateFolderFilePath)(this.project.getPath(), path.join('orgs', this.org.getOrgId(), 'maxRevision.json'), false);
        let maxrevfile;
        try {
        ...
        try {
            const { serverMaxRevisionCounter } = await fs.readJSON(await (0, stateFolderHandler_1.getCurrentStateFolderFilePath)(this.project.getPath(), path.join('orgs', this.org.getOrgId(), 'jayreeStoredMaxRevision.json'), true), { throws: false });
