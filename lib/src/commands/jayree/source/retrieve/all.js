...
See more help with --help`);
            ...
            let out = shell.exec(`sfdx jayree:packagexml --excludemanaged --file=${packageXMLFile} --targetusername=${this.org.getUsername()} --json`, { fatal: false, silent: true, env: Object.assign(Object.assign({}, process.env), { FORCE_COLOR: 0 }) });
            if (config) {
                if (config['source:retrieve:all']) {
            ...
            out = json(shell.exec(`sfdx force:mdapi:retrieve --retrievetargetdir=${orgretrievepath} --unpackaged=${packageXMLFile} --targetusername=${this.org.getUsername()} --json`, { fatal: false, silent: true, env: Object.assign(Object.assign({}, process.env), { FORCE_COLOR: 0 }) }));
            if (out.success) {
                const zip = new AdmZip(out.zipFilePath);
                zip.extractAllTo(orgretrievepath);
                out = json(shell.exec(`sfdx force:mdapi:convert --outputdir=${path.join(orgretrievepath, 'src')} --rootdir=${path.join(orgretrievepath, 'unpackaged')} --json`, {
                    fatal: false,
                    silent: true,
                ...
                if (!this.flags.skipfix) {
                    updatedfiles = await this.applyfixes(config, ['source:retrieve:full', 'source:retrieve:all'], orgretrievepath);
                }
