...
class RetrieveProfiles extends sourceRetrieveBase_1.SourceRetrieveBase {
    async run() {
        ...
        try {
            ...
            pjson.Package.types[pjson.Package.types.findIndex((x) => x.name.toString() === 'CustomObject')].members = pjson.Package.types[pjson.Package.types.findIndex((x) => x.name.toString() === 'CustomObject')].members.concat(config_1.default(this.project.getPath()).ensureObjectPermissions);
            ...
            const out = JSON.parse((await execa('sfdx', ['force:source:retrieve', '--manifest', packagexml, '--targetusername', this.org.getUsername(), '--json'], { cwd: orgretrievepath, env: { FORCE_COLOR: '0', SFDX_DISABLE_JAYREE_HOOKS: 'true' } })).stdout);
            if (((_b = (_a = out === null || out === void 0 ? void 0 : out.result) === null || _a === void 0 ? void 0 : _a.inboundFiles) === null || _b === void 0 ? void 0 : _b.length) > 0) {
                if (this.flags.metadata.includes('Profile')) {
                    inboundFiles.push(...out.result.inboundFiles.filter((x) => x.filePath.includes('force-app/main/default/profiles')));
                }
                if (this.flags.metadata.includes('PermissionSet')) {
                    inboundFiles.push(...out.result.inboundFiles.filter((x) => x.filePath.includes('force-app/main/default/permissionsets')));
                }
                if (this.flags.metadata.includes('CustomLabels')) {
                    inboundFiles.push(...out.result.inboundFiles.filter((x) => x.filePath.includes('force-app/main/default/labels')));
                }
