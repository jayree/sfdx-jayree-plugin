                        ...
                        if (fixsources[filename].delete) {
                            for (const deltask of fixsources[filename].delete) {
                                ...
                                if (deltask.path && deltask.path.match(/(.*\[.*\])/g)) {
                                    deltaskpath = new object_path_1.ObjectPathResolver(data).resolveString(deltask.path).value();
                                }
                                else if (typeof deltask.path === 'undefined') {
                                    deltaskpath = new object_path_1.ObjectPathResolver(data).resolveString(deltask).value();
                                }
                                else {
                                    deltaskpath = delpath();
                                }
                        ...
                        if (fixsources[filename].set) {
                            fixsources[filename].set.forEach((settask) => {
                                ...
                                if (settask.path.match(/(.*\[.*\])/g)) {
                                    settaskpath = new object_path_1.ObjectPathResolver(data).resolveString(settask.path).value();
                                }
                                else {
                                    settaskpath = setpath();
                                }
